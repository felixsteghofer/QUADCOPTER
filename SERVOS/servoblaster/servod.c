...
static char *default_p1_pins = "7,16,18,22";
...
init_hardware(void)
{
	if (delay_hw == DELAY_VIA_PWM) {
		...
		udelay(100);
		clk_reg[PWMCLK_DIV] = 0x5A000000 | (250<<12);	// set pwm div to 250, giving 1MHz
		udelay(100);
		clk_reg[PWMCLK_CNTL] = 0x5A000016;		// Source=PLLD and enable
		udelay(100);
		pwm_reg[PWM_RNG1] = step_time_us *2;
		udelay(10);
...
main(int argc, char **argv)
{
	...
	if (step_time_arg) {
		...
		if (*step_time_arg < '0' || *step_time_arg > '9' ||
				(*p && strcmp(p, "us")) ||
				step_time_us < 1 || step_time_us > 1000) {
